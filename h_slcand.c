...
int set_lint_val(enum gpiod_line_value value)
{
			...
			if (line_cfg)
			{
				...
				if (!ret)
				{
					...
					if (req_cfg)
					{
						...
						{
							syslogger(LOG_NOTICE, "failed to request line '%s' on a GPIO chip '%s'!\n", GPIO_LINE_NAME, GPIO_CHIP_PATH);
						}
					}
					else
					{
	...
	{
		syslogger(LOG_NOTICE, "failed to find line '%s' on a GPIO chip '%s'!\n", GPIO_LINE_NAME, GPIO_CHIP_PATH);
	}
...
int main(int argc, char *argv[])
{
	...
	{
		syslogger(LOG_NOTICE, "failed to get latency flags for device \"%s\": %s!\n", tty, strerror(errno));
	}

	snew.flags |= ASYNC_LOW_LATENCY;
	if (ioctl(fd, TIOCSSERIAL, &snew) < 0)
	{
		syslogger(LOG_NOTICE, "failed to set latency flags for device \"%s\": %s!\n", tty, strerror(errno));
	}
	...
	{
		syslogger(LOG_NOTICE, "sailed to reset attributes for device \"%s\": %s!\n", tty, strerror(errno));
		exit(EXIT_FAILURE);
	}
